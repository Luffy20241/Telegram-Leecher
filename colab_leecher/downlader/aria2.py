...
async def aria2_Download(link: str, num: int):
    ...
    Messages.status_head = f"<blockquote><b> DOWNLOADING FROM » </b><i>Link {str(num).zfill(2)}</i>\n\n<b> Name » </b><code>{name_d}</code></blockquote>\n"
    ...
    if exit_code != 0:
        if exit_code == 3:
            logging.error(f"<blockquote>The Resource was Not Found in {link}</blockquote>")
        elif exit_code == 9:
            logging.error(f"<blockquote>Not enough disk space available</blockquote>")
        elif exit_code == 24:
            logging.error(f"<blockquote>HTTP authorization failed.</blockquote>")
        else:
            logging.error(
                f"<blockquote>aria2c download failed with return code {exit_code} for {link}.\nError: {error_output}</blockquote>"
            )
...
async def on_output(output: str):
    ...
    try:
        ...
        logging.error(f"<blockquote>Could't Get Info Due to: {do}</blockquote>")
    ...
    if elapsed_time_seconds >= 270 and not Aria2c.link_info:
        logging.error("<blockquote>Failed to get download information ! Probably dead link 💀</blockquote>")
