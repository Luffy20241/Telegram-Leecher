...
async def g_DownLoad(link, num):
    ...
    down_msg = f"<blockquote><b>📥 DOWNLOADING FROM » </b><i>🔗Link {str(num).zfill(2)}</i>\n\n<b>🏷️ Name » </b><code>{Messages.download_name}</code>\n</blockquote>"
...
async def getIDFromURL(link: str):
    if "folders" in link or "file" in link:
        ...
        if res is None:
            await cancelTask("<blockquote>G-Drive ID not found in Link.</blockquote>")
            logging.error("<blockquote>G-Drive ID not found.</blockquote>")
            return
        else:
            return res.group(3)
...
async def gDownloadFile(file_id, path):
    ...
    try:
        ...
        err = "<blockquote>Sorry, the specified file or folder does not exist or is not accessible.</blockquote>"
        logging.info(err)
        await cancelTask(err)
        return
    else:
        if file["mimeType"].startswith("application/vnd.google-apps"):
            err = "<blockquote>Sorry, the specified ID is for a Google Docs, Sheets, Slides, or Forms document. You can only download these types of files in specific formats.</blockquote>"
            logging.info(err)
            await cancelTask(err)
            return
        else:
            try:
                ...
                if error.resp.status == 403 and "User Rate Limit Exceeded" in str(
                    ...
                    logging.error("<blockquote>Download quota for the file has been exceeded.</blockquote>")
                    await cancelTask("<blockquote>Download quota for the file has been exceeded.</blockquote>")
                    return
                else:
